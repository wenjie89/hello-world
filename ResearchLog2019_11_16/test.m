...
V(:,1) = pi*(delr/2)^2*delz;
for j=2:size(V,2)
    V(:,j) = pi * ( (r(j)+delr/2 )^2 - ( r(j-1)+delr/2 )^2 )  * delz;
end
...
potential_test = zeros(size(v,1),1);

for i = 1:length(potential_test)
    potential_test(i) = RightPotential(z(i),Lr,Z,R,S,V);
end

%potential_ana2 = zeros(size(v,1),1);


potential_ana2 =  rou0 * r0^3/(3*eps0) ./ sqrt( (z-z0).^2+Lr^2 ) ; 


figure
clf
plot(z,potential_ana2,'r');
hold on 
plot(z,potential_test,'b--');
...
figure(6)
clf
semilogy(z,E(:,find(r==0)))
hold on 
semilogy(z,E_ana_z,'r--')
xlabel('z (m)')
...
figure(7)
clf
semilogy(r,E(find(z==z0),:))
hold on 
semilogy(r,E_ana_r,'r--')
xlabel('r (m)')
ylabel('electric field (V/m)')
legend('Numerical solution', 'Analytical solution')
