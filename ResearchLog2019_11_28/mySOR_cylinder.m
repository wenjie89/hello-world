function [f, it, af] = mySOR_cylinder(f,r,z,rou,w,epsilon)
...
% McGraw-Hill, 1981, pages 174-181
    ...
    while (1)
        ...
        % boundary
        for i = 2:size(f,1)-1 % the bug is fixed!
            r_ = b_*f(i,2)+c_*f(i-1,1)+d_*f(i+1,1)+f(i,1)-s(i,1);
            f(i,1) = f(i,1) - r_ * w;
            norm_r = norm_r + r_^2;
        end
        
        af = norm_r/norm_s;   % factor of accuracy
        ...
        if  norm_r/norm_s < epsilon    % precision requirement is satisfied
            % fprintf('accuracy requirement is satisfied.\n', epsilon);
            break;
        end
        ...
        % condition (3)
        if it > itMax 
             % fprintf('Stopping condition of SOR is not satisfied.\n norm_r/norm_s = %f \n',norm_r/norm_s);
             warning('The solver does not converge after %d iterations.\n', itMax);
             fprintf('Stopping condition of SOR is not satisfied.  norm_r/norm_s = %f \n',norm_r/norm_s);
            break;
